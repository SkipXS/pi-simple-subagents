...
`run_role_agent` calls are serialized inside an orchestration run. For new implementation work packages, omit `workerId` so the tool assigns the next worker session. After a worker completes, the orchestrator is prompted to run a fresh `verifier` (`purpose=validation`) against that package before starting reviewer review. If verification finds concrete implementation gaps, the orchestrator should run `worker` with `purpose=fix` and the same `workerId`, then verify again. For accepted fixes after reviewing a package, pass that package's `workerId`; if omitted for worker fix/validation, the latest worker is reused. If `workerProfiles.light` is configured, the orchestrator may pass `workerProfile: "light"` for small, bounded, low-risk worker tasks; omitting `workerProfile` uses the normal/default `roles.worker`. A worker's first effective profile is sticky per `workerId`: follow-up calls with the same `workerId` may omit `workerProfile` to reuse the binding, but an explicit different profile is rejected. Use a new worker package/`workerId` when you need a different profile. Explicit `workerProfile: "light"` is rejected when the light profile is unconfigured or disabled; omit `workerProfile` to use the default worker. The orchestrator is prompted to verify and review after each implementation package by default; if it starts another implementation worker before the latest package is marked cleanly reviewed, the tool emits a soft batching warning and the orchestrator should record the rationale in `orchestration.md`. Use `/work-parallel` only when tasks are intentionally independent and can use isolated child run directories.
...
Project config overrides global config with deep-merge semantics for nested objects: omitted fields keep earlier defaults/global values. For example, project `workerProfiles: {}` is a no-op for an inherited global light profile, and project `workerProfiles.light` may override just `thinking` or `timeoutMs` while keeping the inherited `model`. A newly configured `workerProfiles.light` must include `model`; partial light-profile overrides are valid only when an inherited profile already supplies the missing fields. Set project `"workerProfiles": { "light": null }` to disable/remove an inherited global light profile; then explicit `workerProfile: "light"` is rejected as not configured. Omit `workerProfile` to use the default worker. Project config is not allowed to set `children.piCliPath` because that value is executed as a child process. Put CLI path overrides in `PI_SIMPLE_SUBAGENTS_PI_CLI` or the global config only when you trust that environment/global profile.
...
| `workerProfiles.light.model` | unset | Optional alternate model for orchestrator-selected light worker tasks. Required when creating a new light profile; may be omitted only in partial overrides of an inherited global light profile. When unset or disabled with `workerProfiles.light: null` in project config, explicit `workerProfile: "light"` is rejected; omit `workerProfile` to use the default worker. |
| `workerProfiles.light.thinking` | `"auto"` when `light` exists | Optional thinking suffix for the light worker profile. `"auto"` chooses one supported level above the default worker's resolved thinking; for an explicit model whose levels are unknown, this means one level above the default worker fallback (commonly `high` when the default worker is `medium`). |
| `workerProfiles.light.timeoutMs` | unset | Optional timeout override for light worker runs; falls back to `roles.worker.timeoutMs` or `children.timeoutMs`. |
